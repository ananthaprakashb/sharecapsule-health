...
type StaticPage = AppSection | 'privacy' | 'ai-sharing'
...
export default function App() {
  ...
  useEffect(() => {
    const pageTitle = route.page === 'activity' ? getActivityBySlug(route.slug)?.title ?? 'Health activity' : route.page === 'home' ? 'ShareCapsule Health' : route.page === 'ai-sharing' ? 'Share with AI · ShareCapsule Health' : `${route.page.charAt(0).toUpperCase()}${route.page.slice(1)} · ShareCapsule Health`
  ...
  if (route.page === 'progress') return <AppChrome active="progress" onNavigate={navigateSection}><ProgressPage onBack={() => navigate({ page: 'home' })} onShareProgress={() => navigate({ page: 'ai-sharing' })} /></AppChrome>
  ...
  if (route.page === 'activity') {
    ...
    if (!activity) return <AppChrome active="home" onNavigate={navigateSection}><HomePage onOpenActivity={openActivity} onOpenRoutines={() => navigate({ page: 'routines' })} onOpenProgress={() => navigate({ page: 'progress' })} /></AppChrome>
