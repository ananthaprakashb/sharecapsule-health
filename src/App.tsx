...
function routeFromHash(): Route { const hash = window.location.hash.replace(/^#\/?/, ''); if (hash === 'routines') return { page: 'routines' }; if (hash === 'progress') return { page: 'progress' }; if (hash === 'settings') return { page: 'settings' }; if (hash === 'privacy') return { page: 'privacy' }; if (hash === 'ai-sharing') return { page: 'ai-sharing' }; if (hash === 'check-in') return { page: 'check-in' }; if (hash.startsWith('activity/')) return { page: 'activity', slug: hash.slice('activity/'.length) }; if (hash === 'breathe/thirumoolar') return { page: 'activity', slug: 'thirumoolar' }; return { page: 'home' } }
...
export default function App() {
  ...
  if (route.page === 'progress') return <AppChrome active="progress" onNavigate={navigateSection}><ProgressPage onBack={() => navigate({ page: 'home' })} onShareProgress={() => navigate({ page: 'ai-sharing' })} onOpenCheckin={() => navigate({ page: 'check-in' })} onOpenGratitude={() => openActivity(getActivityBySlug('thank-someone')!)} /></AppChrome>
  ...
  if (route.page === 'check-in') return <AppChrome active="home" onNavigate={navigateSection}><VoiceCheckinPage onBack={() => navigate({ page: 'home' })} /></AppChrome>
  if (route.page === 'activity') {
    ...
    if (activity.slug === 'thank-someone') return <AppChrome active="home" onNavigate={navigateSection}><GratitudePage onBack={() => navigate({ page: 'home' })} /></AppChrome>
