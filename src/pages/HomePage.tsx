...
import { getDailyFoundationSummary } from '../storage/foundations'
import type { FoundationId } from '../storage/foundations'
...
export function HomePage({ onOpenActivity, onOpenRoutines, onOpenProgress, onOpenCheckin }: HomePageProps) {
  const summary = getTodaySummary(); const progress = getProgressSummary(); const goals = readGoals(); const gratitude = getGratitudeSummary(); const foundations = getDailyFoundationSummary(); const [favorites, setFavorites] = useState(readFavorites)
  ...
  const foundationCount = foundations.filter((item) => item.active).length
  ...
  function openFoundation(id: FoundationId) {
    if (id === 'reflect') { onOpenCheckin(); return }
    const target = id === 'breathe' ? thirumoolarBreath : id === 'move' ? walkActivity : id === 'learn' ? readingActivity : id === 'restore' ? restoreActivity : gratitudeActivity
  ...
  return <main className="page-shell phase3-home"><header className="app-header"><div><p className="eyebrow">by ShareCapsule</p><h1>{APP_SHORT_NAME}</h1><p>{APP_TAGLINE}</p></div><span className="brand-mark" aria-hidden="true">♥</span></header>
    ...
    <section className="phase9-foundations-card" aria-label="Daily foundations"><div className="phase9-foundations-heading"><div><p className="eyebrow">Daily foundations</p><h2>Support the whole day</h2><p>Aim for balance, not perfection. Touch the areas that matter today.</p></div><span><strong>{foundationCount}</strong><small>of 6 supported</small></span></div><div className="phase9-foundation-grid">{foundations.map((item) => <button type="button" key={item.id} className={item.active ? 'active' : ''} onClick={() => openFoundation(item.id)}><span className="phase9-foundation-icon" aria-hidden="true">{item.icon}</span><span><strong>{item.label}</strong><small>{item.detail}</small></span><b aria-label={item.active ? 'Supported today' : 'Not yet supported today'}>{item.active ? '✓' : '○'}</b></button>)}</div></section>
    <section className="phase9-learning-card"><div><span aria-hidden="true">📖</span><div><p className="eyebrow">Learn</p><h2>Read → Recall → Remember</h2><p>Read intentionally, close the source, then retrieve the important ideas from memory.</p></div></div><button className="primary-button" type="button" onClick={() => onOpenActivity(readingActivity)}>Start reading</button></section>
